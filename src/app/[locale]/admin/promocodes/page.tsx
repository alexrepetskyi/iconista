...
export default async function AdminPromoCodesPage() {
  await connectDb();
  const codes = await PromoCode.find().sort({ createdAt: -1 }).limit(200);
  ...
  return (
    <div style={{ display: 'grid', gap: 40 }}>
      <section>
        <h1 style={{ fontWeight: 800, fontSize: 34, marginBottom: 24 }}>Promo codes</h1>
        <div style={{ display: 'grid', gap: 10 }}>
          ...
          {codes.length === 0 ? <p style={{ color: 'var(--stone)', fontWeight: 300 }}>No codes yet.</p> : null}
        </div>
      </section>

      <section style={{ background: '#fff', padding: '28px 26px', maxWidth: 560 }}>
        <h2 style={{ fontWeight: 700, fontSize: 20, marginBottom: 6 }}>New promo code</h2>
        <p className="label-xs" style={{ color: 'var(--stone)', marginBottom: 18 }}>
          Single-use. Optionally bound to one account by email.
        </p>
